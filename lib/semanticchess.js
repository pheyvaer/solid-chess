...
class SemanticChessGame {
  ...
  */
  constructor(url, userDataURL, userWebId, opponentWebId = null, colorOfUser = 'white', chessGame = null, lastMove = null, lastUserMove = null, name = null) {
    this.url = url;
        ...
        if (res.status === 404) {
          ...
          parser.parse(body, async (err, quad, prefixes) => {
            if (err) {
              ...
              const semanticGame = new SemanticChessGame(gameUrl, userDataUrl, userWebId, oppWebId, colorOfUser, chess, lastMove, null, name);
              let t = 'white';
