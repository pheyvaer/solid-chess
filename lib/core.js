...
class SolidChessCore {
  ...
  async getInboxUrl(webId) {
    if (!this.inboxUrls[webId]) {
      this.inboxUrls[webId] = (await data[webId].inbox).value;
    }
    ...
    ?invitation a <${namespaces.schema}InviteAction>.
      ...
      .then(function (result) {
        result.bindingsStream.on('data', async function (result) {
          ...
          if (!gameUrl) {
            gameUrl = this.getGameFromInvitation(invitationUrl);
  ...
  async getGameOfMove(moveUrl) {
    return this.getLiteralFromPredicateForResource(moveUrl, namespaces.schema + 'subEvent');
  }
  ...
  async getGameFromInvitation(url) {
    return this.getLiteralFromPredicateForResource(url, namespaces.schema + 'event');
  }
   ...
   */
  async getLiteralFromPredicateForResource(url, predicate) {
    const deferred = Q.defer();
  ...
  async checkForNewMove(semanticGame = null, userWebId, fileurl, userDataUrl, dataSync, fetch, callback) {
    ...
    if (originalMove) {
      let gameUrl = await
      data[originalMove][namespaces.schema + 'subEvent'];

      if (!gameUrl) {
        gameUrl = await
        this.getGameOfMove(originalMove, fetch);
        ...
        if (game && game.isOpponentsTurn()) {
          ...
          if (nextMoveUrl) {
            ...
            if (semanticGame && game.getUrl() === semanticGame.getUrl()) {
              let san = await
              data[nextMoveUrl][namespaces.chess + 'hasSANRecord'];
  ...
  async processGameToJoin(game, fileurl) {
    ...
    game.name = await
    data[game.gameUrl]['http://schema.org/name'];
