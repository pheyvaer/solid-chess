  ...
  getRDFjsSourceFromUrl(url) {
    ...
    this.fetch(url)
      .then(async res => {
        ...
        this.logger.warn(`No RDFJSSource was created for ${url}. File already deleted?`);
        deferred.resolve(null);
    });
    ...
    if (originalMove) {
      ...
      if (gameUrl) {
        ...
        if (game && game.isOpponentsTurn()) {
          ...
          if (lastMoveUrl) {
            const r = await
              this.getNextHalfMove(fileurl, lastMoveUrl.url, game.getUrl());
            nextMoveUrl = r.move;
            endsGame = r.endsGame;
          } else {
            nextMoveUrl = await
              this.getFirstHalfMove(fileurl, game.getUrl());
          }
