...
async function getName(webid) {
  ...
  engine.query(`SELECT ?firstname ?lastname ?fullname {
    OPTIONAL {<${webid}> <${namespaces.foaf}name> ?fullname.}
    OPTIONAL {<${webid}> <${namespaces.foaf}givenName> ?firstname.}
    OPTIONAL {<${webid}> <${namespaces.foaf}familyName> ?lastname.}
  }`,
    { sources: [ { type: 'rdfjsSource', value: rdfjsSource } ] })
    .then(function (result) {
      result.bindingsStream.on('data', async function (data) {
...
async function getSAN(move) {
  ...
  engine.query(`SELECT ?san {
    <${move}> <${namespaces.chess}hasSANRecord> ?san.
  }`,
    { sources: [ { type: 'rdfjsSource', value: rdfjsSource } ] })
    .then(function (result) {
      result.bindingsStream.on('data', function (data) {
