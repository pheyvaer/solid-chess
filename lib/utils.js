 ...
 */
async function getGamesToContinue(webid, fetch) {
  const deferred = Q.defer();
  const rdfjsSource = await getRDFjsSourceFromUrl(webid, fetch);
 ...
 */
async function getStorageForGame(fileurl, gameUrl, fetch) {
  const deferred = Q.defer();
 ...
 */
async function getNextHalfMove(fileurl, move, gameUrl, fetch) {
  const deferred = Q.defer();
  const rdfjsSource = await getRDFjsSourceFromUrl(fileurl, fetch);
  const engine = newEngine();
  let moveFound = false;
 ...
 */
async function getFirstHalfMove(fileurl, gameUrl, fetch) {
  const deferred = Q.defer();
  const rdfjsSource = await getRDFjsSourceFromUrl(fileurl, fetch);
  const engine = newEngine();
  let moveFound = false;
 ...
 */
async function getJoinRequest(fileurl, userWebId, fetch) {
  const deferred = Q.defer();
  const rdfjsSource = await getRDFjsSourceFromUrl(fileurl, fetch);
  const engine = newEngine();
 ...
 */
function getRDFjsSourceFromUrl(url, fetch) {
  const deferred = Q.defer();
 ...
 */
async function fileContainsChessInfo(fileUrl, fetch) {
  const deferred = Q.defer();
  const rdfjsSource = await getRDFjsSourceFromUrl(fileUrl, fetch);
  const engine = newEngine();
 ...
 */
async function getSANRecord(moveUrl, fetch) {
  const deferred = Q.defer();
  const rdfjsSource = await getRDFjsSourceFromUrl(moveUrl, fetch);
  const engine = newEngine();
 ...
 */
async function getResponseToInvitation(fileurl, fetch) {
  const deferred = Q.defer();
  const rdfjsSource = await getRDFjsSourceFromUrl(fileurl, fetch);
  const engine = newEngine();
 ...
 */
async function getGameOfMove(moveUrl, fetch) {
  return getLiteralFromPredicateForResource(moveUrl, namespaces.schema + 'subEvent', fetch);
}
 ...
 * @returns {Promise}: a promise that returns the url of the game (NamedNode) or null if none is found.
 */
async function getGameFromInvitation(url, fetch) {
  return getLiteralFromPredicateForResource(url, namespaces.schema + 'event', fetch);
}
 ...
 */
async function getLiteralFromPredicateForResource(url, predicate, fetch) {
  const deferred = Q.defer();
  const rdfjsSource = await getRDFjsSourceFromUrl(url, fetch);
  const engine = newEngine();
 ...
 */
async function checkForNewMove(semanticGame = null, userWebId, fileurl, userDataUrl, dataSync, fetch, callback) {
  const originalMove = await getOriginalHalfMove(fileurl, fetch);

  if (originalMove) {
    let gameUrl = await data[originalMove][namespaces.schema + 'subEvent'];

    if (!gameUrl) {
      gameUrl = await getGameOfMove(originalMove, fetch);
    ...
    if (gameUrl) {
      ...
      if (!game || game.getUrl() !== gameUrl) {
        gameStorageUrl = await getStorageForGame(userWebId, gameUrl, fetch);
      ...
      if (game && game.isOpponentsTurn()) {
        ...
        if (lastMoveUrl) {
          const r = await getNextHalfMove(fileurl, lastMoveUrl.url, game.getUrl(), fetch);
          nextMoveUrl = r.move;
          endsGame = r.endsGame;
        } else {
          nextMoveUrl = await getFirstHalfMove(fileurl, game.getUrl(), fetch);
        }
        ...
        if (nextMoveUrl) {
          ...
          if (semanticGame && game.getUrl() === semanticGame.getUrl()) {
            ...
            if (!san) {
              san = await getSANRecord(nextMoveUrl, fetch);
...
async function setUpNewGame(userDataUrl, userWebId, opponentWebId, startPosition, name, dataSync, fetch) {
  const gameUrl = await generateUniqueUrlForResource(userDataUrl);
  const semanticGame = new SemanticChess({url: gameUrl, moveBaseUrl: userDataUrl, userWebId, opponentWebId, name, startPosition});
  const invitation =  await generateInvitation(userDataUrl, semanticGame.getUrl(), userWebId, opponentWebId);

  dataSync.executeSPARQLUpdateForUser(userDataUrl, `INSERT DATA {${semanticGame.getMinimumRDF()} \n <${gameUrl}> <${namespaces.storage}storeIn> <${userDataUrl}>}`, fetch);
  dataSync.executeSPARQLUpdateForUser(userWebId, `INSERT DATA { <${gameUrl}> <${namespaces.schema}contributor> <${userWebId}>; <${namespaces.storage}storeIn> <${userDataUrl}>.}`, fetch);
  dataSync.executeSPARQLUpdateForUser(userDataUrl, `INSERT DATA {${invitation.sparqlUpdate}}`, fetch);
  dataSync.sendToOpponentsInbox(await getInboxUrl(opponentWebId), invitation.notification, fetch);
...
async function processGameToJoin(game, fileurl) {
  ...
  game.opponentsName = await getFormattedName(game.opponentWebId);
  return game;
}
 ...
 * @returns {Promise<void>}
 */
async function joinExistingChessGame(gameUrl, invitationUrl, opponentWebId, userWebId, userDataUrl, dataSync) {
  const loader = new Loader();
  const semanticGame = await loader.loadFromUrl(gameUrl, userWebId, userDataUrl);
  const response = await generateResponseToInvitation(userDataUrl, invitationUrl, userWebId, opponentWebId, "yes");
  ...
  dataSync.sendToOpponentsInbox(await getInboxUrl(opponentWebId), response.notification);
