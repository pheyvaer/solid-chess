 ...
 * @returns {Promise<void>}
 */
async function joinExistingChessGame(gameUrl, invitationUrl, opponentWebId, userWebId, userDataUrl, dataSync, fetch = auth.fetch) {
  const loader = new Loader();
  ...
  dataSync.executeSPARQLUpdateForUser(userWebId, `INSERT DATA { <${gameUrl}> <${namespaces.schema}contributor> <${userWebId}>; <${namespaces.storage}storeIn> <${userDataUrl}>.}`, fetch);
  dataSync.sendToOpponentsInbox(await getInboxUrl(opponentWebId), response.notification, fetch);
