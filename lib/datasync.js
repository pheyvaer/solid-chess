...
class DataSync {
   ...
   * @returns {Promise}: the promise from auth.fetch().
   */
  deleteFileForUser(url, fetch = auth.fetch) {
    return fetch(url, {
      method: 'DELETE'
    });
  }
   ...
   * @returns {Promise}: the promise from auth.fetch().
   */
  executeSPARQLUpdateForUser(url, query, fetch = auth.fetch) {
    return fetch(url, {
      method: 'PATCH',
      body: query,
      headers: {
   ...
   * @returns {Promise}: the promise from auth.fetch().
   */
  sendToOpponentsInbox(url, data, fetch = auth.fetch) {
    return fetch(url, {
      method: 'POST',
      body: data
    });
   ...
   */
  async checkUserInboxForUpdates(inboxUrl, fetch) {
    const deferred = Q.defer();
    const newResources = [];
    //console.log(fetch);
    const rdfjsSource = await Utils.getRDFjsSourceFromUrl(inboxUrl, fetch);
